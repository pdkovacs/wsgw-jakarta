            ...
            for (int i = 0; i < nrClients; i++) {
                connectExec.submit(() -> {
                    try {
                        ...
                        tcLogger.warn("Connect failure", t);
                    }
                    return null;
                });
            }
            ...
            for (ClientTestCtx ctx : processed) {
                sendExec.submit(() -> {
                    ...
                    try {
                        ...
                        tcLogger.with("connId", connId).warn("Client->app send failure", t);
                    }
                    return null;
                });
            }
                ...
                for (int i = 0; i < nrMessagesToSend; i++) {
                    ...
                    Thread.ofVirtual().start(() -> {
                        try {
                            ...
                            transportFailures.add(t);
                            tcLogger.with("connId", client.connectionId()).warn("App->client push failure", t);
                        } finally {
                            inFlight.release();
                        }
                    });
